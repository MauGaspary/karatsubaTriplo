...
public class KaratsubaTriplo {
	...
	// The index in this case is the "m". It will count backwards from the last
	// (least significant) digit and split the string there.
	// It will return a 2-element array of the split string.
	// For example: Given 12345 as the string and 2 as the index, it will split the
	// string into the string array ["123", "45"].
	...
	// An exponent function. Works the same way as Math.pow, but with 64bit integers
	// instead of double precision floats.
	 ...
	 * We'll pick base = 10, and m to be half the length of the digits of the
	 * numbers in this implementation of the algorithm.
	 * In this case, m will be 2, so 10^2 = 100. We will split the 2 numbers using
	 * this multiplier.
	 * The form we want is:
	 * x = x1*b^m + x0
	 * y = y1*b^m + y0
	 ...
	 * Otherwise:
	 * Let z2 = karatsuba(x1, y1). x1 and y1 are the most significant digits, and
	 * are the local variables "high".
	 * Let z0 = karatsuba(x0, y0). x0 and y0 are the least significant digits, and
	 * are the local variables "low".
	...
	public static String karatsuba(String x, String y) {
		...
		if (xString.length() == 1 || yString.length() == 1) {
			...
			// If one of the strings starts with zero, just multiply them normally
		...
		// the algorithm on the parts.
			...
			String[] high1 = strCopy(m2, xString), // the most significant digits. this is the scalar multiplier for
													// b^m2
					low1 = strCopy(m2, xString), // the least significant digits. this is what is added on to high1*b^m2
					high2 = strCopy(m2, yString), // same for y
					low2 = strCopy(m2, yString), // same for y
					// Three recursive calls
					z0 = karatsuba(low1, low2), // z0 = x0y0
					z2 = karatsuba(high1, high2), // z2 = x1y1
					z1 = karatsuba((low1 + high1), (low2 + high2)) - z2 - z0; // z1 = (x0 + y1)*(x1 + y0) - z2 - z0,
																				// courtesy of Karatsuba
